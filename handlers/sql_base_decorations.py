...
async def ccm_start(message: types.Message):
    await FSMA_decor.photo.set()
...
async def load_person(message: types.Message, state: FSMContext):
    ...
    if data['grade'] == 'с8' or data['grade'] == 'c8':
        if data['person'] in c8:
            ...
            await sqlite_db.sql_base_decorations_command(state)
...
def register_handlers_unit(dp: Dispatcher):
    dp.register_message_handler(ccm_start, commands=['Украшения'], state=None)
    dp.register_message_handler(load_photo, content_types=['photo'], state=[FSMA_decor.photo])
    dp.register_message_handler(load_name, state=FSMA_decor.name)
    dp.register_message_handler(load_grade, state=FSMA_decor.grade)
    dp.register_message_handler(load_person, state=FSMA_decor.person)
